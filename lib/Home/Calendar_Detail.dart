...
class _Calendar_DetailState extends State<Calendar_Detail> with WidgetsBindingObserver {
        ...
        if (response.statusCode == 200) {
          ...
          if(json['Status']==401){
            ...
            for(var i in json['Data']){
              ...
              {
                'ID' : addData.issue.firstWhere((e) => e['IssueID'] == i['IssueID'], orElse: () => null) == null ? null : addData.issue.firstWhere((e) => e['IssueID'] == i['IssueID'], orElse: () => null)['ID'],
                'IssueID' : i['IssueID'],
      ...
      if (result.isNotEmpty && result[0].rawAddress.isNotEmpty) {
        ...
        if (response.statusCode == 200) {
          ...
          if(json['Status']==401){
            ...
            activityName.text = addData.name;
            activityDescription.text = addData.description;

            startDate = DateTime.parse(addData.startdate);
            txtStartDate.text = DateFormat('dd MMM yyyy').format(startDate);
            startHour = int.parse(addData.starthour);
            startMinute = int.parse(addData.startminute);

            endDate = DateTime.parse(addData.enddate);
            txtEndDate.text = DateFormat('dd MMM yyyy').format(endDate);
            endHour = int.parse(addData.endhour);
            endMinute = int.parse(addData.endminute);

            setState(() {
              for(int i=0;i<activityDetail['Member'].length;i++){
                addData.team.add(<String, String>
                {
                  'ID' : activityDetail['Member'][i]['ID'],
                  'EmployeeID' : ddlEmp.firstWhere((e) => e['Name'] == activityDetail['Member'][i]['Name'] && e['Role'] == activityDetail['Member'][i]['Role'])['EmployeeID'],
                  'Name' : activityDetail['Member'][i]['Name'],
                  'Role' : activityDetail['Member'][i]['Role'],
                  'delete' : 'false',
                });
              }

              for(var i in ddlEmp){
                var matchData = addData.team.firstWhere((e) => e['Name'] == i['Name'] && e['Role'] == i['Role'], orElse: () => null);
                i['ID'] = matchData == null ? null : matchData['ID'];
                i['delete'] = matchData == null ? null : matchData['delete'];
              }
      ...
      if (result.isNotEmpty && result[0].rawAddress.isNotEmpty) {
        ...
        if (response.statusCode == 200) {
          ...
          if(json['Status']==401){
            ...
            await editEmail(json['Data'][0]['JsonData']);
            setState(() {});
          }
        } else {
          showMyDialog(context, "Please Try Again");
  ...
  Future<dynamic> editEmail(dynamic json) async {
    EasyLoading.show(status: 'loading...');
    await GlobalConfiguration().loadFromAsset("app_settings");
    GlobalConfiguration cfg = new GlobalConfiguration();
                          ...
                          child: new TextButton(
                            onPressed: () {
                              SelectDialog.showModal<String>(
                                ...
                                selectedValue: selectActivityType.firstWhere((e) => e['ID'] == addData.type)['name'],
                                backgroundColor: Colors.white,
                                items: List<String>.generate(
                                  selectActivityType.length,
                                      (i) => selectActivityType[i]['name'].toString(),
                                ),
                              ...
                              child: new TextButton(
                                onPressed: () {
                                  SelectDialog.showModal<String>(
                                    ...
                                    selectedValue: ddlProject.firstWhere((e) => e['ID'] == addData.projectid)['ProjectName'],
                                    backgroundColor: Colors.white,
                                    items: List<String>.generate(
                                      ddlProject.length,
                                      (i) => ddlProject[i]['ProjectName'].toString(),
                                    ),
                                    onChange: (String selected) {
                                      setState(() {
                                        ddlIssue = [];
